...
export default function EditMentoringVisitPage() {
  ...
  const fetchMentoringVisit = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setMentoringVisit(visit);
        setSelectedSchool(visit.pilot_school_id);
        setClassInSession(visit.class_in_session || false);
        setHasLessonPlan(visit.teacher_has_lesson_plan || false);
        setFollowedLessonPlan(visit.followed_lesson_plan || false);
        setNumActivities(visit.num_activities_observed || 1);
        setActivity1ClearInstructions(visit.activity1_clear_instructions || false);
        setActivity1FollowedProcess(visit.activity1_followed_process || false);
        setActivity2ClearInstructions(visit.activity2_clear_instructions || false);
        setActivity2FollowedProcess(visit.activity2_followed_process || false);
        ...
        if (visit.pilot_school_id) {
          fetchTeachers(visit.pilot_school_id);
        }
      } else {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (response.ok) {
        message.success('កែសម្រួលកាត់ទុកវិជ្ជាការបានជោគជ័យ');
        router.push(`/mentoring-visits/${params.id}`);
      } else {
        message.error(data.error || 'មានបញ្ហាក្នុងការកែសម្រួលកាត់ទុកវិជ្ជាការ');
      }
    } catch (error) {
      console.error('Error updating mentoring visit:', error);
      message.error('មានបញ្ហាក្នុងការកែសម្រួលកាត់ទុកវិជ្ជាការ');
    } finally {
      setLoading(false);
    }
  };
  ...
  const canEdit = () => {
    ...
    return (
      (session.user.role === 'admin' || 
       (session.user.role === 'mentor' && mentoringVisit.mentor_id === parseInt(session.user.id))) &&
      ...
      <HorizontalLayout>
        <div className="p-6">
          <Alert
            ...
            description="អ្នកមិនមានសិទ្ធិកែសម្រួលកាត់ទុកវិជ្ជាការនេះទេ ឬកាត់ទុកនេះត្រូវបានជាប់សោ"
            type="error"
            showIcon
          />
        </div>
